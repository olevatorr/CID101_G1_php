...
try {
    ...
    if (isset($_FILES['K_URL']) && $_FILES['K_URL']['error'] === UPLOAD_ERR_OK) {
        ...
        // 更新數據庫中的文件名
        $updateSql = "UPDATE KNOWLEDGE SET K_URL = :K_URL WHERE K_ID = :K_ID";
        $updateStmt = $pdo->prepare($updateSql);
        $updateStmt->bindParam(':K_URL', $newFilename);
        $updateStmt->bindParam(':K_ID', $K_ID);
        $updateStmt->execute();
    }
    ...
    echo json_encode(['error' => false, 'msg' => '新增資料成功', 'K_ID' => $K_ID]);
