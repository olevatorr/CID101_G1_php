...
try {
    ...
    ORDER BY 
        F_DATE DESC
        "; // 準備 SQL 查詢語句，從資料庫中選擇所有知識數據
    $FEEDBACK = $pdo->query($sql); // 執行 SQL 查詢調用pdo裡的函式
    $prodRows = $FEEDBACK->fetchAll(PDO::FETCH_ASSOC); // 獲取所有查詢結果行，並以關聯數組的形式返回

    $result = ["error" => false, "msg" => "", "FEEDBACK" => $prodRows]; // 準備成功的 JSON 響應數據
